...
<div id="wrapper">
	...
	if($action == 'none'){
		...
		$q_solicitud = "UPDATE solicitud SET solicitud_estado = 4 WHERE solicitud_id = ".$solicitud_id;
		$update_solicitud = $object->updquery($q_solicitud);
		
		$date = date("Y-m-d");
		
		$q_operacion = "INSERT INTO operacion (operacion_fecha, socio_id, cuenta_ahorro, usuario_id, tipo_operacion, operacion_monto, solicitud_id) VALUES ('$date','".$solicitud['socio_id']."','".$solicitud['cuenta_numero']."', " . $datos_usuario['usuario_id'] . ",'Prestamo','".$prestamo['prestamo_monto']."',".$solicitud_id.")";
